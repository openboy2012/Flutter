...
class BLGetDollPage extends StatefulWidget {
  BLGetDollPage({Key? key}) : super(key: key);
...
class _BLGetDollPage extends State<BLGetDollPage> {
  late num _crystalNumber;
  late String _account;
  late String _password;
  late String _nickname;
  late String _printText = "准备刷分";
  late BLItemInfoResp loginResp;
  late Dio dio;
        ...
        if (platformInfo.name == "IOS") {
          for (BLServerInfo serverInfo in platformInfo.serveList) {
            ...
            if (serverInfo.nickname == _nickname) {
              ...
              blPrintTextView("账号绑定角色:" +
                  serverInfo.nickname +
                  " 结果:" +
                  BLResponse.fromJson(responseBind.data).code.toString());

              Response responseItemInfo =
                  await this.dio.get(BLApi.BL_USER_DOLL_INFO);
              var blItemInfoResp =
                  BLItemInfoResp.fromGetItemJson(responseItemInfo.data);
              BLItemInfo info = blItemInfoResp.itemInfo;
              blPrintTextView("刷分前的信息[积分:" +
                  info.point.toString() +
                  "蓝染娃娃:" +
  ...
  void getDollAction() async {
    ...
    do {
      Response responseGeDoll =
          await this.dio.post(BLApi.BL_GET_DOLL, data: {"type": dollType});
      ...
      if (blItemInfoResp.code == 0) {
        ...
        if (dollType == 3) {
          ...
          if (blItemInfoResp.itemInfo.dolllr != lastInfo.dolllr + 1) {
            ...
            needContinue = false;
            blPrintTextView("刷到环，任务强制结束 [积分:" +
                lastInfo.point.toString() +
                " 蓝染娃娃数量:" +
                lastInfo.dolllr.toString() +
                " 白哉娃娃数量:" +
                lastInfo.dollbz.toString() +
                "]");
          } else {
            lastInfo = blItemInfoResp.itemInfo;
            blPrintTextView("刷到一个蓝染 [积分:" +
                lastInfo.point.toString() +
                " 蓝染娃娃数量:" +
                lastInfo.dolllr.toString() +
                " 白哉娃娃数量:" +
        ...
        blPrintTextView("继续刷分:" +
            blItemInfoResp.code.toString() +
            " 消息内容:" +
            blItemInfoResp.msg);
      }
    } while (needContinue);
  ...
  void exchangeCrystalAction() async {
    ...
    for (int i = 0; i < _crystalNumber; i++) {
      Response responseGetCrystal =
          await this.dio.post(BLApi.BL_GET_CRYSTAL, data: {"type": 3});
      blItemInfoResp = BLItemInfoResp.fromGetDollJson(responseGetCrystal.data);
      if (blItemInfoResp.code == 0) {
        BLItemInfo lastInfo = blItemInfoResp.itemInfo;
        blPrintTextView("兑换一个水晶成功，[积分:" +
            lastInfo.point.toString() +
            " 蓝染娃娃数量:" +
            lastInfo.dolllr.toString() +
            " 白哉娃娃数量:" +
    ...
    return ListView(
      ...
      children: [
        ...
        TextField(
          ...
          decoration: InputDecoration(labelText: '游戏昵称'),
        ),
        loginFlatButton(),
        TextField(
          onChanged: crystalNumberChanged,
          decoration: InputDecoration(labelText: '水晶数量'),
        ),
        getDollFlatButton(),
