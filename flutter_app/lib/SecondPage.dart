...
class _SecondPage extends State<SecondPage> {
  static Map<String, dynamic> serverKeyValues = {
    "18657349251": "流魂街51区",
    "18043172036": "流魂街52区",
    "1010473892": "流魂街103区",
    "1011550377": "流魂街51区"
  ...
  void _blGetPointAction() async {
    ...
    for (Map<String, dynamic> params in this.blAccounts) {
      ...
      Response responseLogin =
          await this.dio.post(BLApi.BL_USER_LOGIN, data: params);
      ...
      if (platformInfo.name == "IOS") {
        for (BLServerInfo serverInfo in platformInfo.serveList) {
          ...
          if (serverInfo.name == serverKeyValues[account]) {
            ...
            ///绑定账号内容
            Response responseBind =
                await dio.post(BLApi.BL_USER_BIND_ROLE, data: params);

            blPrintTextView("当前绑定的游戏昵称:" +
                serverInfo.nickname +
                " 绑定结果:" +
                BLResponse.fromJson(responseBind.data).code.toString());
  ...
  void _blCircleAction() async {
    ...
    for (Map<String, dynamic> params in this.blAccounts) {
      ...
      ///登陆账号
      Response responseLogin =
          await this.dio.post(BLApi.BLZP_USER_LOGIN, data: params);
      int code = BLResponse.fromJson(responseLogin.data).code;
      blPrintTextView("账号登陆状态:" + code.toString());
      ...
      Response responseLogout = await this
          .dio
          .post(BLApi.BLZP_USER_LOGOUT, data: BLApi.BLZP_WX_OPEN_ID);
  ...
  Future doUserGetZPPoint(Dio dio, String account) async {
    ///获取绑定列表
    Response responseServer = await dio.get(BLApi.BLZP_SERVER_LIST,
        queryParameters: {"openId": BLApi.BLZP_WX_OPEN_ID});
    ...
    for (BLServerInfo serverInfo in list!) {
      ...
      if (serverInfo.name == serverKeyValues[account]) {
        ...
        ///绑定账号内容
        Response responseBind =
            await dio.post(BLApi.BLZP_USER_BIND_ROLE, data: params);
  ...
  Future getAllPointThanPlayByUser(dio, String nickname) async {
    ///按照类型开始领取积分
    for (int i = 1; i < 4; i++) {
      Response responsePoint = await dio.post(BLApi.BLZP_GET_CHAGNE,
          data: {"type": i, "openId": BLApi.BLZP_WX_OPEN_ID});
      BLResponse response = BLResponse.fromJson(responsePoint.data);
      blPrintTextView(nickname +
          " 获取转盘机会 类型:" +
          i.toString() +
          " 结果:" +
    ...
    Response baseResp = await dio.get(BLApi.BLZP_USER_BASEINFO,
        queryParameters: {"openId": BLApi.BLZP_WX_OPEN_ID});
    BLUseChanceResp baseInfo = BLUseChanceResp.fromBaseJson(baseResp.data);
    blPrintTextView(nickname +
        " 获取完转盘次数后，可摇号次数:" +
        baseInfo.canDrawCount.toString() +
        " 已经摇号次数:" +
        baseInfo.useDrawCount.toString() +
        " 消息:" +
        baseInfo.msg);
    do {
      Response responseUseChange = await this
          .dio
          .post(BLApi.BLZP_USE_CHNAGE, data: BLApi.BLZP_WX_OPEN_ID);
      BLUseChanceResp changeInfo =
          BLUseChanceResp.fromJson(responseUseChange.data);
      blPrintTextView(nickname +
          " 摇转盘结果:" +
          changeInfo.code.toString() +
          " 消息:" +
          changeInfo.msg +
          " 奖励:" +
          changeInfo.itemName);
      baseResp = await dio.get(BLApi.BLZP_USER_BASEINFO,
          queryParameters: {"openId": BLApi.BLZP_WX_OPEN_ID});
      baseInfo = BLUseChanceResp.fromBaseJson(baseResp.data);
      blPrintTextView(nickname +
          " 可摇号次数:" +
          baseInfo.canDrawCount.toString() +
          " 已经摇号次数:" +
          ...
          " 消息:" +
          baseInfo.msg);
    } while (baseInfo.canDrawCount - baseInfo.useDrawCount > 0);

    // for (int j = 0; j < 3; j++)
    // {
    //   await Future.delayed(Duration(milliseconds: 11000), () async{
  ...
  Future getAllPointByUser(Dio dio, String nickname) async {
    ...
    for (int i = 0; i < 5; i++) {
      Response responsePoint =
          await dio.post(BLApi.BL_GET_POINT, data: {"type": i});
      BLResponse response = BLResponse.fromJson(responsePoint.data);
      blPrintTextView(nickname +
          " getPoint type:" +
          i.toString() +
    ...
    blPrintTextView("当前账号的信息[积分:" +
        info.point.toString() +
        "蓝染娃娃:" +
        info.dolllr.toString() +
        "白哉娃娃:" +
        info.dollbz.toString() +
        "]");
  }
